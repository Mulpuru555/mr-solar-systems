...
const listEl = document.getElementById("adminServiceList");
...
async function getEmployees() {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      if (role === "employee" || role === "technician" || role === "staff") {
...
async function render() {
  ...
  listEl.innerHTML = filtered.map(r => {
    ...
    return `<div class="docItem" style="display:block;margin-bottom:10px;">
      ...
      <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
        ...
        <button type="button" class="action deleteBtn" data-delete-for="${r.id}">Delete</button>
      </div>
    </div>`;
  }).join("");
  ...
  listEl.querySelectorAll("[data-delete-for]").forEach(btn => {
    btn.addEventListener("click", () => deleteRequest(btn.dataset.deleteFor));
  });
}

async function deleteRequest(id) {
  if (!confirm("Permanently delete this service request? This cannot be undone.")) return;

  try {
    await deleteDoc(doc(db, "serviceRequests", id));
    logActivity("Deleted service request", `id=${id}`);
  } catch (err) {
...
async function saveRequest(id, employees) {
  ...
  try {
    if (noteText) {
      ...
      updates.internalNotes = [...existingNotes, { note: noteText, by: "Admin", at: new Date().toISOString() }];
    }
