...
const locSnap = await getDoc(
  doc(db, "settings", (userBranch || "").toLowerCase())
);
...
window.markAttendance = async function () {
  ...
  try {
    ...
    console.error("Secondary attendance log error", err);
  }
...
try {
  const attSnapUser = await getDocs(
    query(
      collection(db, "attendance"),
      where("userId", "==", user.uid)
    )
  );
  attSnapUser.forEach(d => {
    const data = d.data();
    if (data.date) {
      presentDates.add(data.date);
    }
