...
function initInventoryListener() {
  ...
  }, (err) => console.warn("Inventory sync:", err));

  // Realtime Purchase Records
  onSnapshot(collection(db, "purchaseRecords"), (snap) => {
    purchaseList = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    purchaseList.sort((a, b) => String(b.purchaseDate || b.createdAt || "").localeCompare(String(a.purchaseDate || a.createdAt || "")));
    renderPurchaseRecordsTable();
    updateStockMetrics();
  }, (err) => console.warn("Purchase records sync:", err));
  ...
  onSnapshot(collection(db, "distributionRecords"), (snap) => {
    ...
    distributionList.sort((a, b) => String(b.distributeDate || b.createdAt || "").localeCompare(String(a.distributeDate || a.createdAt || "")));
    renderDistributionRecordsTable();
    updateStockMetrics();
  }, (err) => console.warn("Distribution records sync:", err));
}
